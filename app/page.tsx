...
export default function Home() {
  ...
  const simulatorContent = loadSimulatorPage();
  ...
  const ngnNtnDescription =
    simulatorContent.sections.map((s) => s.content).join(" ") || ntnSimulator?.description || "";
        ...
        <div className="min-w-0 flex-1 py-4 lg:py-10">
          <div className="max-w-3xl space-y-6 md:space-y-12">
            <section id="overview" className="scroll-mt-24">
              <h2 className="font-serif text-xl font-bold text-[var(--navy-900)] tracking-tight md:text-2xl">
                Overview
              </h2>
              <p className="mt-3 text-[0.9375rem] leading-[1.75] text-[var(--body-text)] max-w-3xl md:text-base">
              ...
              <div className="mt-3 space-y-6">
                {icubeQ && (
                  <div>
                    ...
                    <p className="mt-1.5 text-[0.9375rem] leading-[1.75] text-[var(--body-text)] md:text-base">
                      {icubeQ.shortDescription}
                    </p>
                    {icubeQ.sections?.map((section, i) => (
                      <p
                        key={i}
                        className="mt-2 text-[0.9375rem] leading-[1.75] text-[var(--body-text)] md:text-base"
                      >
                        {section.content}
                      </p>
                    ))}
                ...
                <div>
                  <h3 className="font-serif text-lg font-semibold text-[var(--navy-900)] tracking-tight">
                    SSTRL / CubeSat Programs
                  </h3>
                  <p className="mt-1.5 text-[0.9375rem] leading-[1.75] text-[var(--body-text)] md:text-base">
                    National CubeSat initiative. ICUBE-1, ICUBE-N, SSS-2A. Building institutional capacity in space
                    systems engineering.
                  </p>
                </div>
                {ntnSimulator && (
            ...
            <section id="current-work" className="scroll-mt-24">
              ...
              <ul className="mt-3 space-y-1 text-[0.9375rem] leading-[1.75] text-[var(--body-text)] md:text-base">
                ...
                {credibilityItems.map((item) => (
                  <li key={item.id}>
                    <a
                      ...
                      className="text-[var(--navy-900)] hover:underline"
                    >
            ...
            <section id="contact" className="scroll-mt-24 pb-6">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-1 text-[0.9375rem] md:text-base">
                <a href={`mailto:${contact.email}`} className="text-[var(--navy-900)] hover:underline">
                  {contact.email}
                </a>
                {contact.phone && (
                  <a
                    href={`tel:${contact.phone.replace(/\s/g, "")}`}
                    className="text-[var(--slate-700)] hover:underline"
                  >
                    {contact.phone}
                  </a>
                )}
                ...
                <div className="mt-2 flex flex-wrap gap-2.5 items-center">
                  {downloads.map((item) => (
                    <a
                      ...
                      href={item.filePath}
                      download
                      className="rounded-md border border-[var(--border)] bg-white px-2.5 py-1 text-[0.8125rem] font-medium text-[var(--navy-900)] transition-colors hover:border-[var(--slate-400)] hover:bg-[var(--off-white)]"
                    >
                      {item.title}
                    </a>
                  ))}
                  <Link
                    href="/downloads"
                    className="text-[0.6875rem] text-[var(--slate-600)] hover:underline ml-1"
                  >
                    All downloads
                  </Link>
                </div>
